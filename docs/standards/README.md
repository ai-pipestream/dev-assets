...
[`03-platform-extensions.md`](03-platform-extensions.md). A reference of every Quarkus extension shipped by `pipestream-platform` and what it auto-configures so downstream projects stop writing scaffolding. Covers `quarkus-apicurio-registry-protobuf`, `pipestream-quarkus-devservices`, `quarkus-dynamic-grpc`, `pipestream-server`, `pipestream-test-support`, and `pipestream-descriptor`.
